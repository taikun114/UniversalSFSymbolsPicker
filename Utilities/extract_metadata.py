...
def get_existing_file_version(file_path):
    """Extracts the bundleVersion from an existing SFSymbolData.swift file."""
    if not os.path.exists(file_path):
        return None
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
            match = re.search(r'static let bundleVersion: String = "([^"]+)"', content)
            if match:
...
def main():
    ...
    existing_version_str = get_existing_file_version(output_file)
    ...
    print(f"Extracting SF Symbols metadata from: {metadata_dir}")
    ...
    categories_raw = [cat for cat in categories_raw if cat.get("key") not in excluded_category_keys]
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f'    static let bundleVersion: String = "{current_app_version}"\n')
        f.write(f'    static let latestYear: String = "{latest_year}"\n\n')

        f.write("    static let versionToYear: [Double: String] = [\n")
        for v in sorted(version_to_year.keys()): f.write(f'        {v}: "{version_to_year[v]}",\n')
        f.write("    ]\n\n")

        f.write("    static let yearToVersion: [String: [String: String]] = [\n")
        for year, versions in sorted(year_to_release.items()):
            f.write(f'        "{year}": [\n')
            for os_name, version in sorted(versions.items()): f.write(f'            "{os_name}": "{version}",\n')
            f.write("        ],\n")
        f.write("    ]\n\n")

        f.write("    static let categories: [[String: String]] = [\n")
        for cat in categories_raw:
            f.write(f'        ["id": "{cat.get("key", "")}", "label": "{cat.get("label", "")}", "icon": "{cat.get("icon", "")}"],\n')
        f.write("    ]\n\n")

        f.write("    static let aliases: [String: String] = [\n")
        for old_name, new_name in sorted(aliases.items()): f.write(f'        "{old_name}": "{new_name}",\n')
        f.write("    ]\n\n")

        f.write("    static let symbolCategories: [String: [String]] = [\n")
        for symbol, cats in sorted(symbol_categories.items()):
            quoted_cats = ', '.join([f'"{c}"' for c in sorted(cats)])
            f.write(f'        "{symbol}": [{quoted_cats}],\n')
        f.write("    ]\n\n")

        f.write("    static let symbolAvailability: [String: [String]] = [\n")
        for year in sorted(year_to_symbols.keys()):
            quoted_symbols = ', '.join([f'"{s}"' for s in sorted(year_to_symbols[year])])
            f.write(f'        "{year}": [{quoted_symbols}],\n')
        f.write("    ]\n\n")

        f.write("    static let searchKeywords: [String: [String]] = [\n")
        for symbol, keywords in sorted(symbol_search.items()):
            safe_keywords = [k.replace('"', '\\"') for k in sorted(keywords)]
            quoted_keywords = ', '.join([f'"{k}"' for k in safe_keywords])
            f.write(f'        "{symbol}": [{quoted_keywords}],\n')
        f.write("    ]\n\n")

        f.write("    static let restrictedSymbols: [String: String] = [\n")
        for symbol, reason in sorted(restricted_symbols.items()):
            safe_reason = reason.replace('"', '\\"')
            f.write(f'        "{symbol}": "{safe_reason}",\n')
        f.write("    ]\n}\n")

    print("Success: SF Symbols data has been extracted correctly.")
